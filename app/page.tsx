...
type DonneesGemini = { generalites?: string[]; points_a_completer?: string[]; [k: string]: unknown };
...
export default function OutilPage() {
  ...
  // Coordonnées de l'agence, identiques sur toutes les fiches (voir /api/moe).
  const [moe, setMoe] = useState<Contact>(MOE_VIDE);
  ...
  // ---- Coordonnées de l'agence, injectées dans toutes les fiches -----------
  // L'architecte n'a pas à ressaisir ses propres informations : on les récupère
  // une fois et on complète la bibliothèque, y compris les fiches déjà créées.
  useEffect(() => {
    let annule = false;
    (async () => {
      try {
        const r = await fetch("/api/moe");
        if (!r.ok) return;
        const m = (await r.json()) as Contact;
        if (annule || !(m?.nom || m?.telephone || m?.email)) return;
        setMoe(m);
        const cs = lireChantiers().map((c) => ({ ...c, contacts: avecMoe(c.contacts, m) }));
        ecrireChantiers(cs);
        setChantiers(cs);
      } catch {
        /* hors ligne : les fiches restent telles quelles */
      }
    })();
    return () => {
      annule = true;
  ...
  function construireFiche(): Chantier {
    return {
      ...
      contacts: avecMoe(chantierCourant!.contacts, moe).map((c, i) => ({
        ...c,
        // Une ligne ajoutée à la volée n'a pas de case cochée : présente par défaut.
        present: presents[i] ?? true,
      })),
    };
  }
  ...
  async function genererDepuisRevision(): Promise<void> {
    ...
    const apportees = points
      .map((p, i) => ({ point: p, saisie: corrections[i].trim() }))
      .filter((x) => x.saisie.length > 0);
    setRevision(null);
    try {
      ...
      if (apportees.length) {
        ...
        const texte =
          ...
          apportees.map((x) => `- ${x.point} → ${x.saisie}`).join("\n");
        donneesFinales = await analyser(texte, fiche);

        // Le modèle laisse parfois tomber une précision sans le signaler. Ce que
        // l'architecte a saisi ne doit jamais disparaître en silence : on ajoute
        // aux généralités, mot pour mot, celles qui manquent à l'appel.
        const oubliees = precisionsOubliees(
          apportees.map((x) => x.saisie),
          donneesFinales,
        );
        if (oubliees.length) {
          donneesFinales = {
...
function ModaleImmeuble({
  ...
  moe: Contact; // coordonnées de l'agence, pré-remplies
  onAnnuler: () => void;
  ...
  const [contacts, setContacts] = useState<Contact[]>(
    immeuble?.contacts?.length
      ? avecMoe(immeuble.contacts, moe).map((c) => ({ ...CONTACT_VIDE, ...c }))
      : [{ ...CONTACT_VIDE, organisme: "SYNDIC" }, { ...CONTACT_VIDE, ...moe }],
  );
              ...
              {contacts.map((c, i) => (
                <div key={i} className="rounded-xl border border-line p-2.5 space-y-2">
                  <div className="flex gap-2 items-center">
                    <div className="select-wrap w-[86px] sm:w-28 shrink-0">
                      <select value={c.groupe} onChange={(e) => majContact(i, "groupe", e.target.value)} className="champ w-full h-11 text-[16px] sm:text-[13.5px]">
                        <option value="MOA">MOA</option><option value="MOE">MOE</option><option value="ENTREPRISE">ENTREPRISE</option>
                      </select>
                    </div>
                  ...
                  <div className="flex gap-2">
                    {/* Zone de texte et non champ simple : l'agence a deux numéros,
                        et un <input> avalerait silencieusement le retour à la ligne. */}
                    <textarea value={c.telephone || ""} onChange={(e) => majContact(i, "telephone", e.target.value)} rows={2} className="champ flex-1 min-w-0 py-2 leading-snug text-[16px] sm:text-[13.5px] resize-y" placeholder="Téléphone (un par ligne)" />
                    <input value={c.email || ""} onChange={(e) => majContact(i, "email", e.target.value)} type="email" className="champ flex-1 min-w-0 h-11 self-start text-[16px] sm:text-[13.5px]" placeholder="E-mail" />
