 ...
 * (public/DIAG_MODEL_SDC.docx) n'est JAMAIS reconstruit — il est ouvert, rempli,
...
import type { Chantier, Conclusion, DonneesCR, Observation } from "./types";
...
function remplirContacts(doc: any, contacts: Chantier["contacts"]): void {
  ...
  // Le modèle de visite n'a pas de bloc ENTREPRISES, contrairement à celui de
  // chantier. On le fabrique en clonant l'en-tête de groupe « MOE » (ligne 4)
  // et sa ligne de données (ligne 5), pour que l'entreprise intervenante
  // figure au tableau au lieu d'être silencieusement perdue.
  const entreprises = contacts.filter((c) => estEntreprise(c));
  if (entreprises.length && rows[4] && rows[5]) {
    const lignes = enfants(tbl, "w:tr");
    const entete = rows[4].cloneNode(true) as El;
    ecrireCellule(doc, enfants(entete, "w:tc")[0], "ENTREPRISES", { gras: true });
    let ancreEnt: El = lignes[lignes.length - 1];
    ancreEnt.parentNode!.insertBefore(entete, ancreEnt.nextSibling);
    ancreEnt = entete;
    for (const c of entreprises) {
      const tr = rows[5].cloneNode(true) as El;
      ancreEnt.parentNode!.insertBefore(tr, ancreEnt.nextSibling);
      ancreEnt = tr;
      remplir(tr, c);
    }
  }
...
// ---------------------------------------------------------------------------
/**
 * La page de garde du modèle de visite imprime déjà « SDC », et les fiches sont
 * couramment nommées « SDC 12 RUE … » par habitude. On retire donc le préfixe du
 * nom avant de le compléter, sinon la page de garde annonce « SDC SDC 12 RUE … ».
 * Un nom réduit au seul mot « SDC » est laissé tel quel.
 */
function sansPrefixeSdc(nom: string): string {
  const sansPrefixe = (nom || "").replace(/^\s*SDC\b[\s.:—–-]*/i, "").trim();
  return sansPrefixe || (nom || "").trim();
}
...
/**
 * Le modèle de chantier porte un tableau « PLANNING » dont la seconde case est
 * laissée vide : l'outil n'a aucune donnée à y mettre, et la section sortait
 * blanche dans chaque compte rendu. On la retire tant qu'il n'y a rien à y
 * écrire — mieux vaut pas de section qu'une section vide.
 */
function retirerPlanningVide(doc: any): void {
  for (const tbl of tous(doc, "w:tbl")) {
    const cellules = tous(tbl, "w:tc");
    if (!cellules.length) continue;
    const libelle = texteDe(cellules[0]).trim().toUpperCase();
    if (libelle !== "PLANNING") continue;
    // Rempli par quelqu'un ? On n'y touche pas.
    const reste = cellules.slice(1).map((c) => texteDe(c).trim()).join("");
    if (reste) return;
...
// Conclusion du CR de chantier. Le modèle de chantier n'a pas de section dédiée,
// contrairement à celui de visite : on écrit donc le sous-titre nous-mêmes, en gras,
// comme les autres sous-titres du corps. Sans ça la synthèse rédigée par le modèle
// était produite puis jetée en silence.
function insererConclusionChantier(doc: any, ancre: El, conclusion?: Conclusion): void {
  const paras = [...(conclusion?.synthese || []), ...(conclusion?.energetique || [])].filter(Boolean);
  if (!paras.length) return;
  const blocs: { texte: string; opt: OptInser }[] = [
    { texte: "Conclusion", opt: { gras: true, taille: 11, espaceAvant: 12 } },
    ...paras.map((texte) => ({ texte, opt: { taille: 10 } as OptInser })),
  ];
...
// ---------------------------------------------------------------------------
/** Le nom qui signe le document : celui de la maîtrise d'oeuvre. */
function nomSignataire(contacts: Chantier["contacts"]): string {
  const moe = (contacts || []).find((c) => (c.groupe || "").toUpperCase() === "MOE");
  return (moe?.nom || "").trim();
}

/** Une entreprise intervenante : cordiste, couvreur, maçon… (groupe ENTREPRISE). */
function estEntreprise(c: Chantier["contacts"][number]): boolean {
  return (c.groupe || "").toUpperCase().startsWith("ENTREPRISE");
}
...
function remplirContactsChantier(doc: any, contacts: Chantier["contacts"]): void {
  ...
  const remplirLigne = (row: El, ct: Chantier["contacts"][number]) => {
    const cells = enfants(row, "w:tc");
  ...
  // MOE : écrite depuis la fiche comme les autres lignes. Le bloc figé du modèle
  // finissait par diverger des vraies coordonnées de l'agence (numéro périmé).
  if (moe.length && rows[5]) remplirLigne(rows[5], moe[0]);

  // ENTREPRISES : ligne « ETS » du modèle (ligne 7), clonée s'il y en a plusieurs.
  const entreprises = contacts.filter((c) => estEntreprise(c));
  if (entreprises.length && rows[7]) {
    let ancreEnt: El = rows[7];
    entreprises.forEach((c, i) => {
      if (i === 0) {
        remplirLigne(rows[7], c);
        return;
      }
      const tr = rows[7].cloneNode(true) as El;
      ancreEnt.parentNode!.insertBefore(tr, ancreEnt.nextSibling);
      ancreEnt = tr;
      remplirLigne(tr, c);
    });
...
function estTexteSignature(p: El): boolean {
  return /compagnie des architectes|architecte dplg/i.test(texteDe(p));
}
...
function deplacerSignatureEnBas(doc: any, finPhotos: El | null = null, signataire = ""): void {
  const ancre = trouver(doc, "photos");
  ...
  // Le signataire vient de la ligne MOE de la fiche (donc de l'environnement) :
  // aucun nom propre n'est écrit en dur, ni ici ni dans le gabarit.
  const nom = signataire.trim();
  if (nom) {
    const virgule = nom.indexOf(",");
    const l1 = pAligne(doc, "right", 24, true);
    l1.appendChild(runFormate(doc, virgule === -1 ? nom : nom.slice(0, virgule + 1), { gras: true, taille: 11 }));
    if (virgule !== -1) l1.appendChild(runFormate(doc, nom.slice(virgule + 1), { taille: 11 }));
    ins(l1);
  }
...
export function genererDocx(
  ...
  // --- Page de garde ---
  completer(doc, trouver(doc, "SDC"), sansPrefixeSdc(chantier.sdc));
  ...
  essayer(() => deplacerSignatureEnBas(doc, finPhotos, nomSignataire(chantier.contacts)));
...
export function genererDocxChantier(
  ...
  // --- Planning : section sans source de données, retirée si vide ---
  essayer(() => retirerPlanningVide(doc));

  // --- Observations, puis conclusion ---
  // Chaque insertion se place juste après l'ancre : on écrit donc la conclusion
  // en premier pour que les observations viennent se glisser avant elle.
  const ancreObs = trouver(doc, "OBSERVATIONS");
  essayer(() => insererConclusionChantier(doc, ancreObs, donnees.conclusion));
  insererObservations(doc, ancreObs, donnees.observations || []);
